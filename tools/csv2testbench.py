...
# Description:
# Python 3 script that convert logic analizer csv captured data (with time control signal and signal values)
...
# Usage:
# python csv2testbench.py > output.txt
...
#csvfilename = 'IOCS_capture.csv' #'CRAMCS_capture.csv'
#csvfilename = 'ADDR_DATA_AS_DTAC_RESET01.csv'
csvfilename = 'ADDR_DATA_AS_DTAC_VRAMCS01.csv'
#400700 $finish;
    ...
    for signal_name in signal_list_names:
        ...
        if(signal_name != time_header_name): #bypass the time signal itself (time_header_name)
            ...
            try:
                        ...
                        if( row[signal_name] != last_signal_value):
                            ...
                            if(signal_name == clk_name):
                                ...
                                if(row[signal_name] == '1' and last_signal_value == '0'): #is a rising edge, transition from 0 to 1
                                    ...
                                    if(first_clock_capture == 1): 
                                        ...
                                        #print HIGH value of NCLK12 (or LOW for CLK12)
                                        print("{0} = 1'b0;".format(clk_name)) 
                                        print("    ", end = '')
                                        
                                        for i in range(sysclk_ratio * 2, 0, -1):
                                        
                                            #print LOW value of NCLK12 (or HIGH for CLK12)
                                            if (i == sysclk_ratio):
                                                print("{0} = 1'b1;".format(clk_name)) #print("{0} = 1'b0;".format(clk_name))
                                                print("    ", end = '')
                            ...
                            ### END OF SYSCLK PROCESSING ###
                                ...
                                if(row[signal_name][0].upper() == 'H'):
                                    if(signal_name == 'DATA'):
                                        print("{0} = 8'{1:s};".format(signal_name, row[signal_name])) 
                                    else:
                ...
                sys.exit('file %s, line %d: %s' % (csvfilename, reader.line_num, e))
            print("\nend")
    print("initial\nbegin\n    #{0}; $finish;\nend".format(float("{0:.{time_nf}}".format(total_time, time_nf = time_nf))))
